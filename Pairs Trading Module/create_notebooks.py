...
def create_workbook():
    ...
    # Header
    cells.append(nbf.v4.new_markdown_cell("# Pairs Trading: Exercise Workbook\n\n**Instructions:** Do not look at the Solutions notebook. Fill in the markdown prompts using only your notes. Fill in the missing code `___` to get the cells working."))
    
    # Section 0: Data
    cells.append(nbf.v4.new_markdown_cell("## Section 0: Data Download\nFirst, we download a sample universe of 10 stocks across different industries."))
...
X_hl = spread_lag
# No constant for this specific OU implementation
...
def create_solutions():
    ...
    # Header
    cells.append(nbf.v4.new_markdown_cell("# Pairs Trading: Solutions & Answer Key\n\nThis notebook contains the complete explanations and functional code for the workbook exercises."))
...
"""))
    ...
    # Section 1
    cells.append(nbf.v4.new_markdown_cell("## Section 1: Stationarity & Unit Root Tests"))
    cells.append(nbf.v4.new_markdown_cell("""### 📝 Answer: The ADF Test
...
If the p-value is < 0.05, we reject the null hypothesis and assume the series is stationary."""))
    cells.append(nbf.v4.new_markdown_cell("""### 📝 Answer: The KPSS Test
...
"""))
    ...
    # Section 3
    cells.append(nbf.v4.new_markdown_cell("## Section 3: Spread Construction & Hedge Ratio"))
    cells.append(nbf.v4.new_markdown_cell("""### 📝 Answer: Log Prices
We use **log prices** because changes in logarithmic prices approximate percentage returns. By modeling the spread of log prices ($Spread = \ln(A) - \beta \ln(B)$), the spread represents the relative percentage difference between the two assets, making the strategy invariant to the absolute nominal price levels of the stocks. It also helps stabilize the variance over time."""))
    cells.append(nbf.v4.new_code_cell("""# Calculate log prices for AAPL and MSFT
...
"""))
    ...
    # Section 4
    cells.append(nbf.v4.new_markdown_cell("## Section 4: Mean Reversion & Half-Life"))
    cells.append(nbf.v4.new_markdown_cell("""### 📝 Answer: Half-Life
...
"""))
    ...
    # Section 5
    cells.append(nbf.v4.new_markdown_cell("## Section 5: Signal Generation with Bollinger Bands"))
    cells.append(nbf.v4.new_markdown_cell("""### 📝 Answer: Bollinger Bands
...
*   **Exit Signal:** We exit the position when the spread crosses the moving average (mean)."""))
    cells.append(nbf.v4.new_code_cell("""# Define lookback window
window = 20
...
sol = create_solutions()
with open(os.path.join(output_dir, 'Pairs_Trading_Solutions.ipynb'), 'w', encoding='utf-8') as f:
    nbf.write(sol, f)
